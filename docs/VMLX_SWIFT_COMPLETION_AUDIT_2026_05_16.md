...
4. The current focused target passes 156 tests in 24 suites in
   ...
   no-reasoning, Mistral3 JANGTQ factory dispatch, Gemma4 VLM guard, cache
   topology, server settings, MTP runtime, JANGTQ/Hadamard/matmul, VL
   shape/MRoPE, ZAYA config, Omni Parakeet/RADIO, and MiniMax resident-expert
   contracts. A full-package clean-worktree pass still waits on the concurrent
   Flux `Package.swift` work to be committed or isolated.
