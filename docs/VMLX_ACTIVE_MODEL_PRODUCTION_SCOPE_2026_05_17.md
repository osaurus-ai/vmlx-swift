...
| Nemotron Omni / Parakeet / RADIO | JANGTQ4 core artifacts under `20260517T170614Z_omni_live_voice_fresh_recheck/` and related Omni rows prove wrapper-token parity, pre-encoded Parakeet shape, raw PCM/pre-encoded paths, and 65-76 tok/s cache-off streaming rows without literal sound-marker leaks. Fresh `20260517T214045Z_nemotron_jangtq_omni_recheck/` passes the JANGTQ 48-token Omni matrix 18/18 with text, image, video/audio LMInput, media salt, hybrid SSM, and BatchEngine rows. Fresh `20260517T2215_nemotron_mxfp4_omni_recheck/` passes the MXFP4 48-token Omni matrix 18/18 with the same core surfaces. Current 2026-05-18 JANGTQ follow-up proves structured image cache hits (`disk 293/293` and `285/339`) and fixes shared sampler order, but strict gradient rows still fail across multiple seeds and greedy. | PARTIAL current for JANGTQ media; PASS for older 48-token JANGTQ core and JANGTQ4/MXFP4 core; PARTIAL for repeated cache-on audio | Independently encoded Parakeet chunks are not concat-safe. Live voice must retain PCM and submit full-snapshot pre-encoded audio or raw PCM at endpoint. Current JANGTQ no-thinking image follow-up and BatchEngine image rows are not production-clean under strict grounding, and repeated cache-on live audio still needs a focused quality/root-cause gate before full live-voice production promotion. |
