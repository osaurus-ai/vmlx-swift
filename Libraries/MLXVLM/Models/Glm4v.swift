...
private enum Glm4vVision {
    ...
    /// returns (B, gN, gM, C)
    static func gridSampleBilinear(_ x: MLXArray, grid: MLXArray) -> MLXArray {
        let B = x.dim(0)
        let H = x.dim(1)
        let W = x.dim(2)
        let C = x.dim(3)
        ...
        let ix = ((gx + 1) * Float(W) - 1) / 2  // (B, gN, gM)
        ...
        // Clamp corner indices into valid range (border padding).
        ...
        let ix0 = clampX(ix0f)
        let ix1 = clampX(ix0f + 1)
        let iy0 = clampY(iy0f)
        let iy1 = clampY(iy0f + 1)

        // Flatten the spatial grid for gathering: x -> (B, H*W, C)
        let xFlat = x.reshaped(B, H * W, C)

        // Linear flat index = y * W + x, for each of the 4 corners. (B, gN, gM)
        func gather(_ iyc: MLXArray, _ ixc: MLXArray) -> MLXArray {
            let flatIdx = (iyc * Int32(W) + ixc).reshaped(B, gN * gM)
            // gather per-batch along axis 1
            var slices = [MLXArray]()
            for b in 0 ..< B {
                let idxB = flatIdx[b]  // (gN*gM,)
        ...
        let v00 = gather(iy0, ix0)
        let v01 = gather(iy0, ix1)
        let v10 = gather(iy1, ix0)
        let v11 = gather(iy1, ix1)

        let w00 = (wy0 * wx0)[.ellipsis, .newAxis]
        let w01 = (wy0 * wx1)[.ellipsis, .newAxis]
        let w10 = (wy1 * wx0)[.ellipsis, .newAxis]
        let w11 = (wy1 * wx1)[.ellipsis, .newAxis]
    ...
    fileprivate class VisionEmbeddings: Module {
        ...
        func callAsFunction(
            ...
            let interp = Glm4vVision.gridSampleBilinear(pos2dNHWC, grid: grid)
