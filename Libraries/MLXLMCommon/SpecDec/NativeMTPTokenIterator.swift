...
struct NativeMTPTokenIterator: TokenIteratorProtocol {
    ...
    mutating func storeCacheAfterGeneration(
        ...
        let line = String(
            format:
                "[NativeMTP] depth=%d verifyCalls=%d outputTokens=%d acceptedByDepth=%@ bonus=%d rejected=%d avgCommittedPerVerify=%.2f cacheMode=private-mtp+backbone-rollback-repair\n",
            depth,
            verifyCalls,
    ...
    private mutating func verifyCycle() throws {
        ...
        let verifier = model.nativeBackboneForward(input, cache: cache)
