...
final class DeepseekV4FP32LMHeadCacheState {
    ...
    func prepare(
        ...
        // The cache accepts only affine heads. Pass the loaded mode explicitly
        // so this derived weight stays identical to the exact fallback path.
    ...
    func logits(_ hidden: MLXArray, lmHead: Linear) -> MLXArray {
        ...
        // Shadow the exact materialized path. The normal dispatcher now
        // defaults to fused qmm, whose expected rounding difference would make
        // a raw-word cache comparison report false mismatches.
...
public enum DeepseekV4Math {
    ...
    /// Materialize the exact fp32 head for the explicit exact mode and for
    /// raw-word validation of the opt-in cache. This bypasses the qmm default
    /// on purpose.
    internal static func lmHeadExactFp32(_ h: MLXArray, lmHead: Linear) -> MLXArray {
        if let q = lmHead as? QuantizedLinear {
            let weight = MLX.dequantized(
                q.weight, scales: q.scales, biases: q.biases,
                groupSize: q.groupSize, bits: q.bits, mode: q.mode
            ).asType(.float32)
            return lmHeadFp32WithCachedWeight(h, lmHead: q, weight: weight)
        }
        return lmHeadFp32WithCachedWeight(
            h, lmHead: lmHead, weight: lmHead.weight.asType(.float32))
    }
    ...
    public static func lmHeadFp32(_ h: MLXArray, lmHead: Linear) -> MLXArray {
        ...
        return lmHeadExactFp32(h, lmHead: lmHead)
    }
