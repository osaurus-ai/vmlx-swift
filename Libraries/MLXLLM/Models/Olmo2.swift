    ...
    private func validateDecodedFields(container: KeyedDecodingContainer<CodingKeys>) throws {
        ...
        guard hiddenSize == attentionHeads * resolvedHeadDimensions else {
            throw DecodingError.dataCorruptedError(
                forKey: .hiddenSize,
                in: container,
                debugDescription: "Olmo2 hidden_size must equal num_attention_heads * head_dim.")
        }
        guard attentionHeads % kvHeads == 0 else {
            throw DecodingError.dataCorruptedError(
                forKey: .kvHeads,
                in: container,
                debugDescription: "Olmo2 num_attention_heads must be divisible by num_key_value_heads.")
        }
        if let factor = ropeScaling?["factor"]?.asFloat() {
            guard factor.isFinite, factor > 0 else {
                throw DecodingError.dataCorruptedError(
                    forKey: .ropeScaling,
                    in: container,
                    debugDescription: "Olmo2 rope_scaling.factor must be finite and > 0.")
            }
        }
    }
    ...
    private func validatePositive(
        ...
        guard value > 0 else {
            throw DecodingError.dataCorruptedError(
                forKey: key,
                in: container,
                debugDescription: "Olmo2 \(key.rawValue) must be > 0.")
        }
    }
    ...
    private func validatePositive(
        ...
        guard value.isFinite, value > 0 else {
            throw DecodingError.dataCorruptedError(
                forKey: key,
                in: container,
                debugDescription: "Olmo2 \(key.rawValue) must be finite and > 0.")
        }
    }
}
