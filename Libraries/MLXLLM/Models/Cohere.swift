...
public struct CohereConfiguration: Codable, Sendable {
    ...
    private func validateDecodedFields(
        container: KeyedDecodingContainer<CohereConfiguration.CodingKeys>
    ) throws {
        ...
        try validatePositive(logitScale, key: .logitScale, in: container)

        guard hiddenSize % attentionHeads == 0 else {
            throw DecodingError.dataCorruptedError(
                forKey: .hiddenSize,
                in: container,
                debugDescription: "Cohere hidden_size must be divisible by num_attention_heads.")
        }
        guard attentionHeads % kvHeads == 0 else {
            throw DecodingError.dataCorruptedError(
                forKey: .kvHeads,
                in: container,
                debugDescription: "Cohere num_attention_heads must be divisible by num_key_value_heads.")
        }
        if let factor = ropeScaling?["factor"]?.asFloat() {
            guard factor.isFinite, factor > 0 else {
                throw DecodingError.dataCorruptedError(
                    forKey: .ropeScaling,
                    in: container,
                    debugDescription: "Cohere rope_scaling.factor must be finite and > 0.")
            }
        }
    }
    ...
    private func validatePositive(
        ...
        guard value > 0 else {
            throw DecodingError.dataCorruptedError(
                forKey: key,
                in: container,
                debugDescription: "Cohere \(key.rawValue) must be > 0.")
        }
    }
    ...
    private func validatePositive(
        ...
        guard value.isFinite, value > 0 else {
            throw DecodingError.dataCorruptedError(
                forKey: key,
                in: container,
                debugDescription: "Cohere \(key.rawValue) must be finite and > 0.")
        }
    }
}
