...
WHY A GLM-5.3 ORACLE SITS IN A DEEPSEEK V4 FIX
----------------------------------------------
The bug this oracle caught is in `DeepseekV4HyperConnection`, and GLM-5.3 is not yet
implemented here — so the name looks like a leftover.  It is not, and the connection is
the whole reason the bug was findable:

  * The defect is that ONE epsilon was used for two things: `hc_eps` for the sigmoid /
    softmax / Sinkhorn arithmetic, and — wrongly — for the unweighted input RMS norm,
    which the reference takes from `rms_norm_eps`.
  * DeepSeek V4 ships `hc_eps == rms_norm_eps == 1e-6`.  For that family the two are
    interchangeable, the wrong one produces bit-identical output, and NO DeepSeek fixture
    could ever have caught it.  The conflation is unobservable from inside DSV4.
  * GLM-5.3 ships `rms_norm_eps=1e-5` and `hc_eps=1e-6`.  It is, so far, the only family
    that separates them, so its reference is the only source of a fixture that can tell
    the two apart at all.

A GLM-5.3 port is in progress and will reuse `DeepseekV4HyperConnection` on the claim
that the two families share this mechanism.  That claim is what the oracle checks, and
checking it is what exposed the epsilon.  When GLM-5.3 lands, this file is already the
oracle for it; until then it earns its place by being the only thing that can fail when
the epsilons are confused.
