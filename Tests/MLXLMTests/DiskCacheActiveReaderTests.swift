...
struct DiskCacheActiveReaderTests {
    @Test func retainedArraysSurviveQuotaEvictionAndClear() throws {
        try MLXMetalTestLock.withLock {
            ...
            cache.store(tokens: tokens, arrays: [
            ...
            cache.store(tokens: [4, 5, 6], arrays: [
    ...
    @Test func mediaIdentityRemainsIsolatedAfterEvictionAndRestart() throws {
        try MLXMetalTestLock.withLock {
            ...
            writer.store(tokens: tokens, arrays: ["keys": MLXArray.ones([16_384])], mediaSalt: "image-a")
            Thread.sleep(forTimeInterval: 1.1)
            writer.store(tokens: tokens, arrays: ["keys": MLXArray.ones([16_384]) * 2], mediaSalt: "image-b")
