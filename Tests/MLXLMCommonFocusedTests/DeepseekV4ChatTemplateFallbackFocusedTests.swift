...
struct DeepseekV4ChatTemplateFallbackFocusedTests {
    ...
    func nemotronRequiredToolChoiceKeepsNativeXMLToolContract() throws {
        ...
        #expect(rendered.contains("Every parameter listed in a function's `<required>` entry"))
        #expect(rendered.contains("Empty function bodies are invalid tool calls."))
        #expect(rendered.contains("include all required parameters from the user's request"))
