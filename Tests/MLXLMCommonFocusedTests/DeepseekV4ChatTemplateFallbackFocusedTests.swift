...
struct DeepseekV4ChatTemplateFallbackFocusedTests {
    ...
    func lfm2FallbackHonorsRequiredNamedToolChoice() throws {
        ...
        #expect(rendered.contains("Copy the `text` value exactly from the current user request, including newlines and spacing inside the quotes."))
        #expect(rendered.contains("Render newline characters inside the quoted argument as literal \\n escape sequences, not as physical line breaks."))
        ...
        #expect(rendered.contains("Do not output JSON, an empty `line_count()`, or prose."))
    ...
    func lfm2FallbackRepeatsExactRequiredToolValueAfterHistory() throws {
        ...
        #expect(beforeFinalUser.contains("Copy the `text` value exactly from the current user request, including newlines and spacing inside the quotes."))
        #expect(beforeFinalUser.contains("Render newline characters inside the quoted argument as literal \\n escape sequences, not as physical line breaks."))
